...
read.chain.data <- function(file) readRDS(file)
...
save.ggplot(
  ggplot(chain.metadata %>% dplyr::group_by(Folder) %>% dplyr::summarise(n = n()), aes(x = Folder, y = n)) +
    geom_col() +
...
save.ggplot(
  ggplot(chain.metadata, aes(x = Folder, y = nContigs)) +
...
save.ggplot(
  ggplot(chain.metadata, aes(x = Folder, y = nStomata)) +
...
save.ggplot(
  ggplot(chain.contigs, aes(x = Folder, y = length)) +
...
save.ggplot(
  ggplot(contig.totals, aes(x = Folder, y = n)) +
...
save.ggplot(
  ggplot(chain.metadata, aes(x = Folder, y = fUnassignedStomata)) +
